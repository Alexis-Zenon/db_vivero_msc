...
export default function AltaAdministradores({ usuarioActivo, onRegistroExitoso }) {
  ...
  const verificarYAutorizarRol = async () => {
    ...
    try {
      const resValida = await fetch("http://localhost:3000/usuarios/verificar-password", {
        method: "POST",
  ...
  const enviarFormulario = async (e) => {
    ...
    try {
      const respuesta = await fetch("http://localhost:3000/usuarios/registrar-personal", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
