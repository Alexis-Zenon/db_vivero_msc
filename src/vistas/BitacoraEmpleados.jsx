...
export default function BitacoraEmpleados() {
  ...
  const obtenerPersonal = async () => {
    try {
      const res = await fetch("https://db-vivero-msc.onrender.com/usuarios/lista-personal");
      if (res.ok) {
  ...
  const ejecutarEliminacion = async (id, nombre) => {
    ...
    try {
      const res = await fetch(`https://db-vivero-msc.onrender.com/usuarios/${id}`, { method: "DELETE" });
      if (res.ok) {
