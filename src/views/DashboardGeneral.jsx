...
export default function DashboardGeneral({ usuario, onLogout }) {
  ...
  const obtenerDatosDelVivero = async () => {
    try {
      // Carga de temporadas
      try {
        const resTemporadas = await fetch('http://localhost:3000/temporadas');
      ...
      // Carga de plantas
      try {
        const resPlantas = await fetch('http://localhost:3000/plantas');
        if (resPlantas.ok) {
  ...
  useEffect(() => {
    const cargaInicial = async () => {
      ...
      try {
        const resConfig = await fetch('http://localhost:3000/config-ticket');
        if (resConfig.ok) {
  ...
  const guardarConfiguracionLocalidad = async (nuevosDatos) => {
    try {
      const respuesta = await fetch('http://localhost:3000/config-ticket', {
        method: 'POST',
